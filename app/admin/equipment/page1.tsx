...
export default function AdminEquipmentPage() {
  ...
  const [form, setForm] = useState({ name: '', description: '', serialNumber: '', fixedAssetId: '', responsibleUserId: '', spaceId: '' });
  ...
  const fetchInitialData = useCallback(async () => {
    ...
    try {
      ...
      if (spacesRes.ok) {
        ...
        setError(prev => (prev ? prev + ' | ' : '') + 'Error al cargar espacios.');
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An unknown error occurred');
    } finally {
  ...
  const handleCloseModal = () => {
    ...
    setForm({ name: '', description: '', serialNumber: '', fixedAssetId: '', responsibleUserId: '', spaceId: '' });
    setExistingImages([]);
    setSelectedFiles(null);
    setError(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ ...form, images: uploadedImageUrls }),
      });
      if (!response.ok) {
        const errorData = await response.json();
  ...
  return (
    <Container fluid style={{ paddingTop: '100px' }}>
      ...
      <Row className="d-none d-md-flex align-items-center mb-3">
        ...
        <Col md={9} className="text-end">
          <div className="d-flex justify-content-end gap-2">
            ...
            <Button variant="secondary" onClick={() => { /* Download logic here */ }}>Descargar CSV</Button>
              ...
              {equipment.map(item => (
                <tr key={item.id}>
                  ...
                  <td>
                    ...
                    <Button variant="danger" size="sm" onClick={() => handleDelete(item.id)} disabled={item.id === user.id}>Eliminar</Button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal show={showModal} onHide={handleCloseModal}>
        <Modal.Header closeButton><Modal.Title>{currentEquipment ? 'Editar Equipo' : 'Añadir Nuevo Equipo'}</Modal.Title></Modal.Header>
        <Modal.Body>
